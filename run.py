...
if __name__ == "__main__":
    ...
    # Si estamos en un entorno de producción (como Railway) con Gunicorn, 
    # no deberíamos ejecutar app.run() aquí ya que Gunicorn se encarga de Flask.
    # Sin embargo, para que el bot corra, necesitamos que este script se ejecute.
    # Railway usualmente ejecuta el comando del Procfile.
    
    print(f"[RUN] Servidor intentando escuchar en el puerto {port}")
    try:
        ...
        print(f"[RUN] No se pudo iniciar Flask (posiblemente ya iniciado por Gunicorn): {e}")
